...
import { deleteLoyaltyProgramAction } from "@/app/clients/loyalty-program-actions";
...
export function DeleteLoyaltyProgramDialog({
  open,
  onOpenChange,
  clientId,
  program,
  ...
  const handleConfirm = async () => {
    ...
    setPending(true);
    try {
      const result = await deleteLoyaltyProgramAction(clientId, program.id);
      if (!result.ok) {
          ...
          >
            ...
            <Dialog.Description className="mt-2 text-sm text-fora-muted">
              {program ? (
                <>
                  This removes{" "}
                  <span className="font-medium text-fora-navy">{program.programName}</span> from
                  the client profile. You can ask the client to send details again if needed.
                </>
              ) : null}
            </Dialog.Description>
